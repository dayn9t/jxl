...
def main():
    ...
    parser.add_argument(
        "coco_file", metavar="COCO_FILE", type=Path, help="COCO格式文件"
    )
    parser.add_argument(
        "darknet_dir", metavar="DARKNET_DIR", type=Path, help="Darknet样本标注目录"
    )
    parser.add_argument(
        "-c", "--cats", nargs="+", type=str, default=None, help="选中的类别，默认全部"
    )
    parser.add_argument(
        "-r",
        "--rect",
