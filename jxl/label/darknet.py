...
def img2label(image_file: Path) -> Path:
    """从图片文件获取标注文件"""
    label_file = (image_file.parent.parent / 'labels' / image_file.name).with_suffix('.txt')
    assert image_file.exists()
    assert label_file.exists()
    return label_file


def darknet_dump_labels(labels: ImageLabelPairs, folder: Path, meta: LabelMeta, crop_roi: bool = False,
                        keep_dst_dir: bool = False, prefix: str = '') -> int:
