...
class Labeler(RecordViewer):
    ...
    def __init__(self, meta: LabelMeta, meta_id: int, snapshot_dir: Path, verbose: bool):
...
def main() -> None:
    ...
    parser.add_argument('-l', '--label_filter', type=int, default=2,
                        help='样本标注过滤：1-所有样本，2-可导入标注样本，3-已标注样本')
    parser.add_argument('-c', '--conf_thr', type=float, default=1.0, help='置信度阈值, 标注错误置信度为-2')
    ...
    rs = load_label_records(opt.folder, opt.meta_id, LabelFilter(opt.label_filter), opt.pattern, opt.conf_thr)
