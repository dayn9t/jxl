...
def main() -> None:
    ...
    parser.add_argument('-k', '--keep_dst_dir', action='store_true', help='保留目标目录, 不重建')
    parser.add_argument('-C', '--crop_roi', action='store_true', help='裁剪ROI, 只对检测任务有效')
    ...
    if opt.prop:
        ...
        total = dump_label_prop(labels, dst_dir, cat_id, opt.prop, opt.keep_dst_dir, opt.prefix)
    else:
        total = darknet_dump_labels(labels, dst_dir, meta, opt.crop_roi, opt.keep_dst_dir, opt.prefix)
    print(f"\n样本({total})生成完毕!")
