...
def main():
    ...
    parser.add_argument('-N', '--non_normalized', action='store_true', help='放弃数据归一化')
    parser.add_argument('-f', '--data_format', type=int, default=0, help='数据格式：0-完整模型，1-参数包')
    parser.add_argument('-m', '--max-prob', type=float, default=1, help='概率上限')
    parser.add_argument('--category', type=int, default=-1, help='类别，取值：[0,N]，默认不限制')
    ...
    with torch.no_grad():  # 不计算导数
        ...
        cls_opt = ClassifierOpt((opt.img_size, opt.img_size),
                                opt.num_classes,
