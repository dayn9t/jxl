...
def make_model(cfg: Config, device: str, model_file: Path) -> MMDataParallel:
    ...
    model1: MMDataParallel = build_dp(model, cfg.device, device_ids=cfg.gpu_ids)
...
class OpenCD(ISeg):
    ...
    def __init__(self, model_path: Path, opt: SegOpt, device: str = "cuda") -> None:
    ...
    def forward_np(self, images: list[np.ndarray]) -> np.ndarray:
        ...
        tensor: Tensor = data_in["img"].unsqueeze(0)
