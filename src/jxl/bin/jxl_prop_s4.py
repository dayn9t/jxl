...
def main() -> None:
    ...
    parser.add_argument("folder", type=Path, help="图片标注目录")
    parser.add_argument("meta_id", type=int, help="元数据ID")
    parser.add_argument("category", type=int, help="类别ID")
    parser.add_argument("property", type=int, help="属性ID")
    parser.add_argument("-c", "--exclude-conf", type=float, default=1.0, help="排除置信度阈值")
    parser.add_argument("-m", "--min-prop", type=int, default=0, help="最小属性值")
    parser.add_argument(
        "-s",
        "--sort-by-conf",
        type=int,
        default=0,
        help="排序方式: 0=按值, 1=置信度升序, 2=置信度降序",
    )
