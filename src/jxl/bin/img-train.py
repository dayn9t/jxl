...
# - 查看修改：git diff HEAD~100 main.py
...
parser.add_argument(
    "--num_classes", default=1000, type=int, metavar="N", help="number of total classes"
)
parser.add_argument("--non_normalized", action="store_true", help="不归一化数据")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not args.non_normalized:
        trans.append(normalize)
    trans_queue = transforms.Compose(trans)
    train_dataset = datasets.ImageFolder(traindir, trans_queue)
    ...
    val_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(valdir, trans_queue),
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.workers,
        pin_memory=True,
...
def save_checkpoint(state, is_best, model, filename="checkpoint.pth.tar"):
    ...
    if is_best:
        ...
        name = "best"
        shutil.copyfile(filename, name + ".pth.tar")
