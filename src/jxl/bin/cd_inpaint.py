...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="变化检测(CD)样本绘制工具", epilog=epilog
    )
    ...
    parser.add_argument(
        "-c", "--size", type=str, default="1024x1024", help="目标图片尺寸"
    )
    parser.add_argument("-m", "--multiple", type=int, default=5, help="目标样本倍数")
    parser.add_argument(
        "-o", "--object_num", type=int, default=4, help="图片内绘制目标数量"
    )
    ...
    blender = ObjectBlender(
        multiple=opt.multiple, ob_count=opt.object_num, size=size, verbose=opt.verbose
    )
