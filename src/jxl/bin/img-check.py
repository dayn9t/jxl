...
def main() -> None:
    ...
    parser.add_argument(
        "--img-size", type=int, default=224, help="输入图像尺寸，默认224"
    )
    parser.add_argument("-n", "--num_classes", type=int, help="类别数")
    parser.add_argument(
        "-N", "--non_normalized", action="store_true", help="不归一化数据"
    )
    parser.add_argument(
        "-r", "--review", action="store_true", help="复核错误 & 不可信数据"
    )
    parser.add_argument(
        "-c", "--max_conf", type=float, default=0.95, help="复核时筛选的最大置信度"
    )
    parser.add_argument("-i", "--class_id", type=int, help="只处理指定类别")
    parser.add_argument(
        "-t", "--top_num", type=int, default=20, help="复核时每个类别最多显示的样本条目"
    )
