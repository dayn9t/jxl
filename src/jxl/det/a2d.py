...
def prop_to_label(
    ...
    Returns: (prop_id, ProbValue) — properties 现以 int prop_id 为键 (jvi-migration)。
    """
    _ = prop_names[prop_id]
    max_conf = max(probs)
    max_index = probs.index(max_conf)
    return prop_id, ProbValue(max_index, max_conf)
...
def props_to_label(
    ...
    """将属性概率分布集合转换为标签和值集合"""
    label_props: dict[int, ProbValue] = {}
    for prop_id, probs in props.items():
        _pid, prob_value = prop_to_label(prop_id, probs, prop_names)
        label_props[prop_id] = prob_value
    return label_props
...
class A2dResult(BaseModel):
    ...
    def to_label(self, names: list[str]) -> A2dImageLabel:
        """
        ...
        """
        objects = [ob.to_label(names) for ob in self.objects]
