...
from jvi.geo.rectangle import Rect, random_point
from jvi.geo.size2d import Size
from jvi.image.image_nda import ImageNda, ImageNdas
from jvi.image.io import load_image_pairs_in, load_images_in
...
def _lerp(x: float, x0: float, y0: float, x1: float, y1: float) -> float:
    """线性插值: f(x0)=y0, f(x1)=y1."""
    if x1 == x0:
        return y0
    return y0 + (y1 - y0) * (x - x0) / (x1 - x0)


def _sample_excluding[T](items: list[T], n: int, exclude: T) -> list[T]:
    """从 items 中不放回随机抽取 n 个, 跳过 exclude 项."""
    rest = [it for it in items if it != exclude]
    return sample(rest, min(n, len(rest)))
...
def random_object_pos(size: Size, size_ob: Size) -> Rect:
    ...
    for _i in range(times):
        ...
        # 归一化边长: y=0 时为 y0_side_n, y=size.height 时为 y1_side_n
        side_n = _lerp(p.y, 0, y0_side_n, size.height, y1_side_n)
...
class ObjectBlender:
    ...
    def _make_sample(
        ...
        if self._verbose:
            logger.info(f"#{self._count} save: {dst_dir / 'x' / name}")
    ...
    def make_samples(self, src_dir: StrPath, dst_dir: StrPath) -> int:
        ...
        for path, im1 in pairs:
            ...
            pairs1 = _sample_excluding(pairs, n, (path, im1))
            for _, im2 in pairs1:
